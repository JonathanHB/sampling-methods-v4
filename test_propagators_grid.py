...
def test_propagators_grid_1d_cv_shared_grid():
    ...
    init_coords = np.random.uniform(-1.5, 1.5, size=(n_walkers, sim_system.n_dim))
    init_potentials = np.zeros(CV.grid_n)
    ...
    traj, pots, weights = propagate_shared_grid(
        ...
        init_potentials=init_potentials, steps_per_saved_frame=100,
        n_gaussians=300, frames_per_gaussian=10,
        sigma=sigma, omega=omega, delta_T=delta_T,
        CV=CV.cv_funct, grad_CV=CV.cv_grad_funct, 
        cv_min=CV.cv_min, cv_max=CV.cv_max,
        we_weights = [1/n_walkers for _ in range(n_walkers)]
    )
    ...
    plot_trj(traj[0], pots)
    plot_trj(traj[1], pots)
...
def test_propagators_grid_2d_cv_shared_grid():
    ...
    grid_shape2 = (25, 25)
    init_coords = np.random.uniform(-1.5, 1.5, size=(n_walkers, sim_system.n_dim))
    init_potentials2 = np.zeros(grid_shape2)


    traj2, pots2, weights2 = propagate_shared_grid(
        G=sim_system.G, kB=kB, T=T, dt=0.001, xi = sim_system.xi, init_coords=init_coords,
        init_potentials=init_potentials2, steps_per_saved_frame=100,
        n_gaussians=300, frames_per_gaussian=10,
        sigma=sigma2, omega=2, delta_T=40.0,
        CV=CV2.cv_funct, grad_CV=CV2.cv_grad_funct, 
        cv_min=CV2.cv_min, cv_max=CV2.cv_max,
        we_weights = [1/n_walkers for _ in range(n_walkers)]
    )
    ...
    plt.imshow(pots2[-1], extent=(CV2.cv_min[0], CV2.cv_max[0], CV2.cv_min[1], CV2.cv_max[1]), origin='lower', aspect='auto')
    plt.colorbar(label='Bias Potential (kT)')
    plt.title("bias potential at final frame for walker 0 (2 CVs)")
    plt.xlabel('x0=CV0')
    plt.ylabel('x1=CV1')
    ...
    plt.imshow(pots2[150], extent=(CV2.cv_min[0], CV2.cv_max[0], CV2.cv_min[1], CV2.cv_max[1]), origin='lower', aspect='auto')
    plt.colorbar(label='Bias Potential (kT)')
    plt.title("bias potential at frame 150 (halfway) for walker 0 (2 CVs)")
    plt.xlabel('x0=CV0')
    plt.ylabel('x1=CV1')
